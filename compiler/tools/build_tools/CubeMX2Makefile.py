...
LLVM_PATH = None
HEXBOX_LIB = None
HEXBOX_GRAPH_TOOL = None
HEXBOX_FINAL_LD_TOOL = None
HEXBOX_RESULT_TOOL = None
...
def build_LUTS():
    ...
    mcu_regex_to_comp_support_dict = {
            'STM32(F|L)0': 'hexbox-rt-v6-m.o',
            'STM32(F|L)1': 'hexbox-rt-v7-m.o',
            'STM32(F|L)2': 'hexbox-rt-v7-m.o',
        #    'STM32(F|L)3': thumb -mcpu=cortexsp-d16 -mfloat-abi=hard',
            'STM32(F|L)3': 'hexbox-rt-v7e-m.o',
        #    'STM32(F|L)4': thumb -mcpu=cortexsp-d16 -mfloat-abi=hard',
            'STM32(F|L)4': 'hexbox-rt-v7e-m.o',
            'STM32(F|L)7': 'hexbox-rt-v7e-m.o',# -mfloat-abi=hard',
    }
...
def main():
    ...
    if len(sys.argv) != 4:
        ...
        sys.stderr.write("\nUsage:\n")
        sys.stderr.write("  CubeMX2Makefile.py <SW4STM32 project folder> <HEXBOX ROOT> <Project Name>\n")
    ...
    # Configuration
    hexbox_root_path = os.path.abspath(sys.argv[2])
    print hexbox_root_path
    if not (os.path.isdir(hexbox_root_path)):
        sys.stderr.write("HEXBOX \"Root of HexBox Tools not found not found: \n")
        sys.exit(C2M_ERR_INVALID_COMMANDLINE)
    GCC_PATH = os.path.join(hexbox_root_path,'gcc','bins/')
    LLVM_PATH = os.path.join(hexbox_root_path,'llvm','bins')
    HEXBOX_LIB = os.path.join(hexbox_root_path,'llvm','hexbox-rt-lib')
    HEXBOX_GRAPH_TOOL = os.path.join(hexbox_root_path,'compiler','graph_analysis','analyzer.py')
    HEXBOX_FINAL_LD_TOOL  = os.path.join(hexbox_root_path,'compiler','graph_analysis','final_linker_gen.py')
    HEXBOX_RESULT_TOOL = os.path.join(hexbox_root_path,'compiler','graph_analysis','collect_results.py')
    build_LUTS()
    ...
    for mcu_regex_pattern, lib in mcu_regex_to_comp_support_dict.items():
        if re.match(mcu_regex_pattern, mcu_str):
            rt_lib_record = HEXBOX_LIB +"/"+ "record-"+lib
            rt_lib_enforce = HEXBOX_LIB +"/"+ "enforce-"+lib
            break

    for mcu_regex_pattern, lib in mcu_regex_to_syscalls_dict.items():
        if re.match(mcu_regex_pattern, mcu_str):
            syscalls_lib = HEXBOX_LIB +"/"+ lib
            break
    ...
    makefile_str = makefile_template.substitute(
        ...
        GCC_PATH = GCC_PATH,
        LDMCU = ld_subst,
        HEXBOX_RT_LIB_RECORD = rt_lib_record,
        HEXBOX_RT_LIB_ENFORCE = rt_lib_enforce,
        ...
        HEXBOX_GRAPH_TOOL = HEXBOX_GRAPH_TOOL,
        HEXBOX_FINAL_LD_TOOL = HEXBOX_FINAL_LD_TOOL,
        HEXBOX_RESULT_TOOL = HEXBOX_RESULT_TOOL,
